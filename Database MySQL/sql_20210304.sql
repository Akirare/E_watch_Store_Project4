...
-- 23/03/2021 bo dung them cot id cho orderdetails
ALTER TABLE `ewatch_data`.`orderdetails` 
...
CREATE TABLE `ewatch_data`.`comments` (
  ...
  `ProductId` INT NOT NULL,
  `CustomerId` VARCHAR(20) NOT NULL unique,            -- de unique la khong hop ly
  `Comment` TEXT NULL,
...
-- 31/03/2021 sua cau truc comments
CREATE TABLE `ewatch_data`.`comments` (
  `CommentId` INT NOT NULL AUTO_INCREMENT,
  `ProductId` INT NOT NULL,
  `CustomerId` VARCHAR(20) NOT NULL,
...
-- 16/03/2021 bo sung table carts
CREATE TABLE `ewatch_data`.`carts` (
  ...
  PRIMARY KEY (`CartId`),
  CONSTRAINT `CustomerId_Carts`
    FOREIGN KEY (`CustomerId`)
    REFERENCES `ewatch_data`.`customers` (`CustomerId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `ProductId_Carts`
    FOREIGN KEY (`ProductId`)
    REFERENCES `ewatch_data`.`products` (`ProductId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
    
ALTER TABLE `ewatch_data`.`carts` 
DROP COLUMN `Amount`;
...
INSERT INTO `ewatch_data`.`carts` (`CustomerId`, `ProductId`, `Quantity`) VALUES ('cus003', '3', '1');
